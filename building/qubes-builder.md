...
Next, you specify which components (`gcc`, for example) you do *not* want to compile:

    COMPONENTS := $(filter-out gcc,$(COMPONENTS))

Alternatively, you could edit the actual COMPONENTS list which is defined in the included version-dependent config from example-configs (see series of include directives near the beginning of `builder.conf`).
